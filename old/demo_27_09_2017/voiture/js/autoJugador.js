...
(function(){
	...
	function CarPlayer(){
		...
		this.init=function(scene,aspect){
			...
			var cam=new THREE.PerspectiveCamera(65,aspect, 0.1, 3000);
			// this.cam.position.set(0.8,0.5,-0.0);
			cam.lookAt(new THREE.Vector3(0.15,0,5));
			cam.rotation.y=Math.PI;
			cam.position.set(-0.8,0.5,-0.0);
			this._container.add(cam);
			this.cameras.push(cam);
			
			var cam2=new THREE.PerspectiveCamera(65,aspect, 0.1, 3000); 
			cam2.position.set(0,10,-10.0);
			cam2.lookAt(new THREE.Vector3(0.15,1,5));			
			this._container.add(cam2);	
			this.cameras.push(cam2);
...
//			console.log(ang);
	...
	inheritPrototype(CarPlayer, MultiMesh);
	
	CarPlayer.prototype.onAssetsLoaded=function(){
		...
		this._multiMaterials={
			...
			"auto": new THREE.MeshFaceMaterial([
		...
		var internalLight = new THREE.PointLight( 0xffeeee, 0.2, 10 );
		internalLight.position.set( 0, 1, 0 );
		this._container.add(internalLight);				
		
		var ch=new THREE.Mesh(this._geometries["chasis.DAE"],this._multiMaterials["interior"]);
		var vd=new THREE.Mesh(this._geometries["vidrios.DAE"]);		
		var auto=new THREE.Mesh(this._geometries["auto1.DAE"],this._multiMaterials["auto"]);
...
//		console.log(this._container);
	...
	SERIOUSGAME.player=new CarPlayer();
